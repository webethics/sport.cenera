...
const Warderobe = () => {
  ...
  const addWardrobes = async (values: any) => {
    setAdding(true);
    try {
      let WardrobesAdded = warderobe.some(
        (res) => res.wardrobe_name === values.warderobe
      );

      if (!WardrobesAdded) {
        const resposne = await createWardrobes(
        ...
        if (resposne.data.message) {
          ...
          enqueueSnackbar("Wardrobes Added Successfully", {
            variant: "success",
          });
        }
      } else {
  ...
  const handleDelete = async (wardrobe_id: any) => {
    ...
    try {
      ...
      if (response) {
        await revalidate();
        enqueueSnackbar("Wardrobes Deleted Successfully", {
          variant: "success",
        });
        ...
        <Paper className={classes.paper}>
          <Card className={classes.root} variant="outlined">
            <CardContent>
              ...
              <List>
                {warderobe.map(
                  (res: { wardrobe_id: number; wardrobe_name: string }) => (
                    <ListItem
            ...
            <CardActions className={classes.cardActions}>
              ...
              />
              {errors.warderobe && touched.warderobe && (
                <span className={classes.errorColor}>{errors.warderobe}</span>
              )}
              <Button
                type="submit"
                variant="contained"
