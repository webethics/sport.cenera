...
export const GameResults: FC = () => {
  ...
  const { gameInfo, loading, isValidating, revalidate } = useFetchGameInfo(teamId);
    ...
    <div>
      <GridContainer>
        <GridItem xs={12} sm={12} md={8} xl={9} className={classes.container}>
          {showDropDown? <Teampicker teamList={teamsList} onChange={handleTeamChange}  value={teamId} />: ""}
          ...
          <Card>
            ...
            <CardBody>
              {!loading && gameInfo
                ...
                : null}
             {teamId? <MatchResultForm onMatchRecordUpDeleted={revalidate} team_id={teamId}  onMatchRecordUpUpdated={revalidate} /> :   //new
                <MatchResultForm onMatchRecordUpDeleted={revalidate} onMatchRecordUpUpdated={revalidate} /> 
             } 
            </CardBody>
          </Card>
        </GridItem>
